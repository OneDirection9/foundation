...
class Visualizer(object):
   ...
   """
    ...
    def __init__(self, image: np.ndarray, scale: float = 1.0):
        """
        Args:
            image: An RGB image with shape HxWx3.
            scale: Scale the input image.
        """
        if not isinstance(image, np.ndarray):
            raise TypeError('image should be np.ndarray. Got {}'.format(type(image)))
        if image.ndim != 3 or image.shape[-1] != 3:
            raise ValueError('image should be of shape HxWx3. Got {}'.format(image.shape))
        ...
        self._default_font_size = max(np.sqrt(self.height * self.width) // 90, 10 // scale)
    ...
    def get_image(self) -> np.ndarray:
        ...
        image_rgba = buffer.reshape(height, width, 4)
        rgb, alpha = np.split(image_rgba, [3], axis=2)

        try:
            import numexpr as ne  # fuse them with numexpr

            visualized_image = ne.evaluate('image * (1 - alpha / 255.0) + rgb * (alpha / 255.0)')
        except ImportError:
    ...
    def draw_circle(self, circle_coord: PointCoordinate, color: MPLColor, radius: int = 3) -> None:
        """
        Args:
        ...
        """
        self.ax.add_patch(mpl.patches.Circle(circle_coord, radius=radius, fill=True, color=color))
    ...
    def draw_box(
        ...
        *,
        ...
        """
        Args:
            box_coord: A tuple containing x1, y1, x2, y2 coordinates, where x1 and y1 are the
                coordinates of the image's top left corner. x2 and y2 are the coordinates of the
    ...
    def draw_rotated_box(
        ...
        *,
        ...
        line_style: str = '-',
        label: Optional[str] = None,
        small_object_area_thresh: int = 1000,
    ) -> None:
        """
        Args:
            ...
            small_object_area_thresh: Using thinner lines if box area less than
                `small_object_area_thresh`.
        """
        ctr_x, ctr_y, w, h, angle = rotated_box
        area = w * h
        # use thinner lines when the box is small
        linewidth = self._default_font_size / (
            6 if area < small_object_area_thresh * self.scale else 3
        )
    ...
    def draw_binary_mask(
        ...
        *,
        ...
        alpha: float = 0.5,
        label: Optional[str] = None,
        large_mask_area_thresh: int = 120000,
    ) -> None:
        """
        Args:
            ...
            large_mask_area_thresh: Add label in the middle of components if it's area larger than
                `large_mask_area_thresh`.
        """
        ...
        if label is not None:
            ...
            for cid in range(1, _num_cc):
                if cid == largest_component_id or stats[cid, -1] > large_mask_area_thresh:
    ...
    def _create_grayscale_image(self, mask: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        """
        image_bw = self.image.astype('f4').mean(axis=2)
        image_bw = np.stack([image_bw] * 3, axis=2)
        if mask is not None:
            image_bw[mask] = self.image[mask]
        return image_bw
