...
def to_float_tensor(numpy_array: np.ndarray) -> torch.Tensor:
    ...
    if len(numpy_array.shape) == 2:
        ...
        raise NotImplementedError("Unknown numpy_array dimension of {}".format(float_tensor.shape))
    return float_tensor
...
def to_numpy(float_tensor: torch.Tensor, target_shape: list, target_dtype: np.dtype) -> np.ndarray:
    ...
    if len(target_shape) == 2:
        ...
        raise NotImplementedError("Unknown target shape dimension of {}".format(target_shape))
